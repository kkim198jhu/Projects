...
class CTree {
 ...
 public:
  ...
  bool addChild(CTree *ExistingTree);

  std::string toString(); // all characters, separated by newlines, including at the end

  std::string DFtraverse(CTree * tracker, std::string new_string);//Makes it easier to get the toString
 ...
 private:
  ...
  // returns false on any type of failure, including invalid root
  bool addSibling(char d);
  bool addSibling(CTree *new_sibling);
